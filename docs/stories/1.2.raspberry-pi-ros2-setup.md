...
- [ ] Verify directory structure: `ls -la` (should show docs/, firmware/, orchestrator/, etc.)
...
│   ├── config/
│   └── requirements.txt
├── firmware/                # ESP32 firmware (compiled on dev machine, not Pi)
...
6. **Repository Clone Verification:**
   - Command: `ls ~/olaf/`
   - Expected: `docs/`, `firmware/`, `orchestrator/`, `tools/`, etc.
