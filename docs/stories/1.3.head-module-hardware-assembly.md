...
- [ ] **Task 2: Wire SPI display connections for both eyes** (AC: 2)
  - [ ] **Left Eye (CS: GPIO5):**
    ...
    - [ ] Connect GC9A01 SCL (CLK) → ESP32-S3 GPIO12 (SPI SCK - shared)
    - [ ] Connect GC9A01 SDA (MOSI) → ESP32-S3 GPIO11 (SPI MOSI - shared)
    - [ ] Connect GC9A01 RES (RST) → ESP32-S3 GPIO4 (shared)
    - [ ] Connect GC9A01 DC (or A0) → ESP32-S3 GPIO2 (shared)
    - [ ] Connect GC9A01 CS → ESP32-S3 GPIO5 (Left Eye CS)
    - [ ] Optional: Connect GC9A01 LED → ESP32-S3 GPIO10 (backlight control)
  - [ ] **Right Eye (CS: GPIO15):**
    - [ ] Connect GC9A01 VCC → ESP32-S3 3.3V (shared with left)
    - [ ] Connect GC9A01 GND → ESP32-S3 GND (shared)
    - [ ] Connect GC9A01 SCL (CLK) → ESP32-S3 GPIO12 (shared)
    - [ ] Connect GC9A01 SDA (MOSI) → ESP32-S3 GPIO11 (shared)
    - [ ] Connect GC9A01 RES (RST) → ESP32-S3 GPIO4 (shared)
    - [ ] Connect GC9A01 DC (or A0) → ESP32-S3 GPIO2 (shared)
    - [ ] Connect GC9A01 CS → ESP32-S3 GPIO15 (Right Eye CS)
    - [ ] Optional: Connect GC9A01 LED → ESP32-S3 GPIO10 (shared or separate GPIO)
