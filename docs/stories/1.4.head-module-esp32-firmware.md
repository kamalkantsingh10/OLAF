...
- [x] **Task 1: Set up PlatformIO project for head module** (AC: 1)
  - [x] Create `firmware/head/firmware/` directory structure
...
- [x] **Task 7: Code cleanup and documentation** (AC: 7)
  ...
  - [x] Commit code to `firmware/head/firmware/`
...
```
firmware/head/                  # Head Module (I2C 0x08)
├── firmware/
│   ├── head_controller.ino
...
**Key Paths:**
- Firmware code: `firmware/head/firmware/`
- PlatformIO config: `firmware/head/platformio.ini`
- Module documentation: `firmware/head/README.md`
...
**Test Location:** `firmware/head/test/`
...
```bash
# Build firmware
pio run -d firmware/head

# Upload to ESP32
pio run -d firmware/head -t upload
...
**New Files Created:**
- `firmware/head/firmware/i2c_slave.h` - I2C slave interface with register map definitions
- `firmware/head/firmware/i2c_slave.cpp` - I2C slave implementation with interrupt handlers
- `firmware/head/firmware/gc9a01_driver_spi.h` - Dual display driver interface
- `firmware/head/firmware/gc9a01_driver_spi.cpp` - Dual display driver implementation
- `firmware/head/firmware/eye_expression.h` - Expression engine interface
- `firmware/head/firmware/eye_expression.cpp` - Expression engine with 7 emotions and animation

**Modified Files:**
- `firmware/head/src/main.cpp` - Complete rewrite integrating all components for Story 1.4
- `firmware/head/README.md` - Updated with Story 1.4 features, register map, testing procedures

**Existing Files (Unchanged):**
- `firmware/head/platformio.ini` - Build configuration from Story 1.3
- `firmware/head/firmware/config.h` - Hardware configuration from Story 1.3
