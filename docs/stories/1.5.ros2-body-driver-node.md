...
- [ ] **Task 1: Set up ROS2 package structure** (AC: 1)
  ...
  - [ ] Create `head_driver.py` with proper ROS2 node boilerplate
  - [ ] Add dependencies: `rclpy`, `smbus2`, `std_msgs`
  - [ ] Configure node name: `/olaf/head_driver`
...
- [ ] **Task 3: Implement module health check** (AC: 5)
  - [ ] On node startup, read register 0x00 from address 0x08
  - [ ] Verify response is 0x08 (module ID)
  - [ ] Log initialization success/failure
  - [ ] Set internal health status flag

- [ ] **Task 4: Create ROS2 subscribers** (AC: 3)
  - [ ] Subscribe to `/olaf/head/expression` (create custom message or use dict)
  - [ ] Implement callback to write expression_type to register 0x10
  - [ ] Implement callback to write intensity to register 0x11
  - [ ] Subscribe to `/olaf/head/blink` (std_msgs/Empty)
  - [ ] Implement callback to write to register 0x12 (blink trigger)

- [ ] **Task 5: Create ROS2 publisher for status** (AC: 4)
  - [ ] Create publisher `/olaf/head/status` (std_msgs/String)
...
- [ ] **Task 7: Test node functionality** (AC: 7)
  - [ ] Launch node: `ros2 run olaf_drivers head_driver`
...
**Key Paths:**
- Node file: `orchestrator/ros2_nodes/hardware_drivers/head_driver.py`
...
3. Launch node: `ros2 run olaf_drivers head_driver`
